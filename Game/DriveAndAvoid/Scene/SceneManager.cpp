...
{
	//ウィンドウのタイトルを設定
	SetMainWindowText("Dribe&Acoid");

	//ウィンドウモードで起動
	if (ChangeWindowMode(TRUE)!=DX_CHANGESCREEN_OK)
	{
		throw ("ウィンドウモードで起動できませんしました\n");
	}
...
void SceneManager::Update()
{
	...
	while (ProcessMessage()!=-1)
	{
		...
		if (CheckHitKey(KEY_INPUT_SPACE)||InputControl::GetButtonUp(XINPUT_BUTTON_BACK))
		{
			break;
		}
	}
